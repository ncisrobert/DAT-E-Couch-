...
void loop() {
    ...
    /* What would happen if we uncomment this block? */
    //client.publish(publishChannel,buttonState);

    /* this is an example of reading a sensor.
    Here, we both stream data, and check a threshold value to send state changes
    */
    ...
    char buffer [4];
    sprintf (buffer, "%03i", sensorReading);
    client.publish("relax/ball",buffer);
    delay(100);
